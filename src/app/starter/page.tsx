...
const TIPOVI: Record<Tip, {
  ...
  branic: {
    ...
    savjet: 'Definiraj "sigurnosni prag" — iznos koji ti daje mir. Sve iznad toga — investiraj. Novac koji miruje gubi vrijednost (inflacija).',
  },
  vrtlog: {
    emoji: '🌀',
    naziv: 'Vrtlog',
    ...
    savjet: 'Jedan mali korak dnevno je 100% bolji od savršenog plana koji nikad ne počneš. Počni s 5 minuta danas — ništa više.',
  },
  teoreticar: {
    emoji: '📚',
    naziv: 'Teoretičar',
    ...
    savjet: 'Svaki dan JEDNA konkretna stvar. Bez istraživanja, bez čitanja. Implementiraj, evaluiraj, prilagodi. Znanje bez akcije nije znanje.',
  },
}

const DEFAULT_TIP: typeof TIPOVI[Tip] = {
  emoji: '📊',
  naziv: 'Tvoja dijagnoza',
  podnaslov: 'Personalizirani uvid u tvoje financije',
  zdravScore: 44,
  snage: ['Odlučnost za promjenom', 'Svjesnost o problemu', 'Volju za učenjem'],
...
// ─── 30-dnevni planovi po tipu ────────────────────────────────────────────────

type Task = { day: number; task: string; tip_: string }

const PLANOVI: Record<Tip, Task[]> = {
  hedonist: [
    { day: 1,  task: 'Financijski audit: zbrojvi sve prihode i rashode zadnjeg mieseca', tip_: '📊 Temelje ne možeš preskočiti' },
    ...
    { day: 5,  task: 'Postavi "buffer" od 200 € za neočekivane troškove ovog mieseca', tip_: '🛡️ Hitni fond počinje ovdje' },
    ...
    { day: 11, task: 'Definiraj 3 konkretna financijska cilja za sljedeće 6 mieseci', tip_: '🎯 Cilj bez roka je samo želja' },
    { day: 12, task: 'Otkaži još 2 pretplate ili servis koji koristiš rijetko', tip_: '💰 Svaki euro vrijedi' },
    { day: 13, task: 'Izračunaj koliko imaš u hitnom fondu vs. cilj (3× miesečne troškove)', tip_: '🛡️ Koliko si daleko?' },
    ...
    { day: 30, task: 'Napravi financijski plan za sljedeći miesec s konkretnim brojevima', tip_: '🎯 Kraj je novi početak' },
  ],
  branic: [
    { day: 1,  task: 'Financijski audit: zbrojvi sve prihode i rashode zadnjeg mieseca', tip_: '📊 Crno na bijelo' },
    ...
    { day: 4,  task: 'Istraži visoko-prinosni štedni račun — usporedi 3 banke', tip_: '💰 Infacija jede ušteđevinu' },
    ...
    { day: 7,  task: 'Tjedni pregled: je li automatski transfer radiao? Kako se osjećaš?', tip_: '✅ Tjedan 1 završen!' },
    ...
    { day: 12, task: 'Definiraj 3 financijska cilja: kratkoročni, средњoročni, dugoročni', tip_: '🎯 Ciljevi ruše anksioznost' },
    ...
    { day: 22, task: 'Postavi mjsečni automatski doprinos investicijama', tip_: '🤖 Dollar cost averaging' },
    { day: 23, task: 'Istraži nekretnine kao investiciju — plus i minus', tip_: '🏠 Jedna od opcija, ne jedina' },
    { day: 24, task: 'Definiraj "trošaj slobode" — iznos koji možeš trošiti bez analize', tip_: '🎉 Sloboda je i cilj' },
    { day: 25, task: 'Napravi jednu kupnju sa "trošajem slobode" — bez krivnje', tip_: '✨ Zaslužio/la si' },
    ...
    { day: 30, task: 'Napravi financijski plan za sljedeće 3 mieseca s konkretnim brojevima', tip_: '🎯 Planiranje = sigurnost' },
  ],
  vrtlog: [
    { day: 1,  task: 'SAMO ovo: postavi podsjетnik za 9 ujutro svaki dan s natpisom "Dan X"', tip_: '⏰ Struktura je tvoj prijatelj' },
    ...
    { day: 5,  task: 'Napиши 3 razloga zašto TI osobno treba financijska sloboda', tip_: '💡 Tvoje "zašto" je gorivo' },
    ...
    { day: 15, task: 'Napиши financijsкi plan za ovaj tjedan — na komadu papira, na vidljivom miestu', tip_: '👀 Vidljivo = prisutno' },
    { day: 16, task: 'Jedan "mini cilj" za ovaj tjedan: uštedjeti X € specifično', tip_: '🎯 Mali, mjerljivi ciljevi' },
    { day: 17, task: 'Ako si propustio/la nešto ove tjedne — nastavi TAMO gdje si stao/la (ne ispočetka)', tip_: '🔄 Nastavi, ne ponovi' },
    { day: 18, task: 'Istraži ETF fondove — 10 minuta, na YouTubeu, ne čitaj', tip_: '📺 Vizualno učenje brže' },
    { day: 19, task: 'Razgovaraj s bankom o boljem štednom računu ili fиксnoj štednji', tip_: '🏦 Jedna akcija, dugoročan prinos' },
    ...
    { day: 24, task: 'Provjeri: koliko se tvoj finansijski status poboljšao od dana 1?', tip_: '📈 Mjerljivost = motivacija' },
    { day: 25, task: 'Napiši što ti je bio najveći "aha moment" u 30 dana', tip_: '💡 Uvid = trajni temelj' },
    { day: 26, task: 'Postavi financijsku rutinu za sljedeći miesec — isti dani, iste aktivnosti', tip_: '📅 Rutina = sloboda' },
    { day: 27, task: 'Istraži jedan konkretan način povećanja prihoda — side hustle ili pregovaranje', tip_: '💰 Prihodi + štednja = ubrzanje' },
    { day: 28, task: 'Tjedni pregled: 4 tjedna sustava. Što će biti drugačije sljedeći miesec?', tip_: '✅ 4 tjedna — sustav je uspostavljen!' },
    { day: 29, task: 'Napiši pismo sebi za 6 mieseci: gdje želiš biti financijski', tip_: '🔮 Vizija vuče akciju' },
    { day: 30, task: 'Finalni plan: 3 financijska cilja za sljedeće 3 mieseca s konkretnim koracima', tip_: '🎯 Kraj je novi početak' },
  ],
  teoreticar: [
    ...
    { day: 10, task: 'Postavi podsjetnik: svaki 1. u miesecu prebacuješ ratu hitnog fonda', tip_: '📅 Automatizacija eliminira odluke' },
    ...
    { day: 13, task: 'Provjeri postoji li kreditna kartica s boljim uvjetima od tvoje — apliciraй odmah', tip_: '💳 Akcija, ne usporedba 12 opcija' },
    ...
    { day: 15, task: 'Dodaj 50 € više u investicije ovog mieseca — bez analiziranja', tip_: '📈 Svaki euro investiran danas vrijedi više' },
    ...
    { day: 20, task: 'Napiši jednu strvar koju si odgađao/la godinama — napravi je danas', tip_: '⚡ Odlaganje je skup luksuz' },
    ...
    { day: 23, task: 'Pregledaj sve automatizacije — rade li? Prilagodi bez duboке analize', tip_: '🔧 Prilagodba ≠ perfekcionizam' },
    { day: 24, task: 'Napravi financijsku bilancю: imovina minus obveze = neto vrijednost', tip_: '📊 Jedna tablica, sav pregled' },
    { day: 25, task: 'Postavi cilj za neto vrijednost za 1 godinu — napиши ga', tip_: '🎯 Bez izračuna — procijeni intuitivno' },
    ...
    { day: 30, task: 'Plan za sljedeći miesec: 3 konkretne akcije, bez analize', tip_: '🎯 Kraj je novi početak' },
  ],
}
...
function HealthScoreGauge({ score }: { score: number }) {
  ...
  const getColor = (s: number) => s < 40 ? '#ef4444' : s < 60 ? '#f59e0b' : s < 80 ? '#22c55e' : '#D4AF37'
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <p className="text-xs text-center mt-1" style={{ color: 'rgba(255,255,255,0.35)' }}>
        Početni Health Score baziran na tvojem tipu
      </p>
    </div>
  )
}
...
function PlanRow({ task, done, onToggle }: { task: Task; done: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      style={{
        backgroundColor: done ? 'rgba(34,197,94,0.08)' : 'rgba(255,255,255,0.03)',
...
function StarterContent() {
  ...
  useEffect(() => {
    if (!token) { setError('Token nedostaje u URL-u.'); setLoading(false); return }

    fetch(`/api/starter/validate?token=${encodeURIComponent(token)}`)
      .then(r => r.json())
      .then(json => {
        if (json.valid) {
          setData(json)
          ...
          setError(json.error ?? 'Token nije validan.')
        }
        setLoading(false)
      })
      .catch(() => { setError('Greška pri provjeri pristupa.'); setLoading(false) })
  }, [token])
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#0D1B2A' }}>
      <div className="text-center">
        ...
        <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.9rem' }}>Provjeravamo tvoj pristup...</p>
      </div>
    </div>
  )
  ...
  if (error || !data) return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: '#0D1B2A' }}>
      <div className="text-center max-w-sm">
        <div style={{ fontSize: '3rem', marginBottom: 16 }}>🔒</div>
        <h1 className="text-2xl font-black mb-3" style={{ color: '#fff' }}>Pristup nije pronađen</h1>
        <p className="mb-6" style={{ color: 'rgba(255,255,255,0.5)' }}>{error || 'Ovaj link nije validan ili je istekao.'}</p>
        <p className="text-sm mb-4" style={{ color: 'rgba(255,255,255,0.35)' }}>
          Provjeri email koji si dobio/la nakon kupnje — link je tamo.
        </p>
        <Link href="/" className="text-sm font-semibold" style={{ color: '#D4AF37' }}>← Natrag na fincoach.vip</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#0D1B2A' }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50"
        ...
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <span className="text-xs font-bold" style={{ color: '#D4AF37' }}>Starter Paket</span>
        </div>
      </nav>
      ...
      <div className="px-4 pb-24" style={{ paddingTop: 80 }}>
        <div className="max-w-xl mx-auto">
          ...
          <div className="text-center mb-6">
            <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'rgba(212,175,55,0.6)' }}>
              Tvoja financijska dijagnoza
            </p>
          ...
          <div className="flex gap-2 mb-6 p-1 rounded-xl" style={{ backgroundColor: 'rgba(255,255,255,0.04)' }}>
            {(['dijagnoza', 'plan', 'videa'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                style={{
                  backgroundColor: activeTab === tab ? 'rgba(212,175,55,0.2)' : 'transparent',
          ...
          {activeTab === 'dijagnoza' && (
            <div className="space-y-5">
              ...
              <div className="rounded-2xl p-5" style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.1) 0%, rgba(212,175,55,0.04) 100%)', border: '1px solid rgba(212,175,55,0.25)' }}>
                ...
                <Link href={`${siteUrl}/volim-svojnovac`}
          ...
          {activeTab === 'plan' && (
            <div>
              ...
              <div className="rounded-2xl p-5 mb-5" style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                {doneCount > 0 && (
                  <p className="text-xs mt-2" style={{ color: 'rgba(255,255,255,0.35)' }}>
                    Odlično! {doneCount === 30 ? '🎉 Završio/la si cijeli plan!' : `Nastavi — još ${30 - doneCount} dana do kraja.`}
                  </p>
                )}
              </div>

              {/* Tjedni grupovani prikaz */}
              {[0, 1, 2, 3].map(week => {
              ...
              <p className="text-xs text-center mt-4" style={{ color: 'rgba(255,255,255,0.25)' }}>
                Tvoj napredak se sprema lokalno u preglediku.
              </p>
            </div>
          )}
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {video.status === 'soon' && (
                          <span className="text-xs px-2 py-0.5 rounded-full font-bold"
                            style={{ backgroundColor: 'rgba(212,175,55,0.15)', color: '#D4AF37' }}>Uskoro</span>
                        )}
                      </div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#0D1B2A' }}>
        <div className="text-center">
          <div style={{ fontSize: '3rem', marginBottom: 16 }}>🔐</div>
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.9rem' }}>Učitavam tvoj Starter Paket...</p>
        </div>
      </div>
    }>
      <StarterContent />
