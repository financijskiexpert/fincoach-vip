...
const PLANOVI: Record<Tip, Task[]> = {
  hedonist: [
    { day: 1,  task: 'Financijski audit: zbrojvi sve prihode i rashode zadnjeg mjeseca', tip_: '📊 Temelje ne možeš preskočiti', detail: 'Otvori internet bankarstvo ili aplikaciju banke. Zbrojvi sve prihode u jednu kolonu i sve troškove u drugu. Cilj: znati točne brojeve — ne procjene. Procjena je uvijek premala.' },
    ...
    { day: 13, task: 'Izračunaj koliko imaš u hitnom fondu vs. cilj (3× mjesečne troškove)', tip_: '🛡️ Koliko si daleko?', detail: 'Ukupni månedni troškovi × 3 = minimalni hitni fond. Primjer: 1.200€/mj × 3 = 3.600€ cilj. Koliko imaš trenutno na štednom računu? Razlika = put koji gradiš ovim programom.' },
    ...
    { day: 25, task: 'Smanji jedan fiksni trošak za 10% (internet, mobitel, osiguranje)', tip_: '📞 Jedno pregovaranje tjedno', detail: 'Nazovi pružatelja mobitela, interneta ili osiguranja. Reci: "Vidim povoljniju ponudu drugdje — možete li se uskladiti?" 10% na 3 fiksna troška = 30-60€ trajne uštedine svaki måsec, zauvijek.' },
  ...
  branic: [
    { day: 1,  task: 'Financijski audit: zbrojvi sve prihode i rashode zadnjeg mjeseca', tip_: '📊 Crno na bijelo', detail: 'Otvori internet bankarstvo ili aplikaciju banke. Zbrojvi sve prihode u jednu kolonu i sve troškove u drugu za prošli måsec. Ovo nije za osudu — samo za jasnu sliku stvarnosti. Crno na bijelo.' },
    ...
    { day: 12, task: 'Definiraj 3 financijska cilja: kratkoročni, srednjoročni, dugoročni', tip_: '🎯 Ciljevi smanjuju anksioznost', detail: 'Kratkoročni (do 1 god) + Középoročni (1-5 god) + Dugoročni (5+ god). Svaki ima drugačiji rizičan profil. Format: ŠTO + KOLIKO€ + DO KADA. Konkretni ciljevi smanjuju financijsku anksioznost jer daju smjer.' },
    ...
    { day: 22, task: 'Postavi automatski månedni doprinos investicijama', tip_: '🤖 Dollar cost averaging u akciji', detail: 'Trajni nalog za investiranje: isti datum (npr. 5. u mj.), isti iznos svaki måsec. Automatski doprinos koji se uvijek dogodi > promjenjivi doprinos koji ovisi o raspoloženju. Dosljednost pobjeđuje iznos.' },
    ...
    { day: 24, task: 'Definiraj "trošak slobode" — iznos koji možeš trošiti bez analize', tip_: '🎉 Sloboda je i cilj', detail: 'Definiraj iznos koji možeš potrošiti svaki måsec na što god hoćeš — bez analize ili krivnje. Može biti 50€ ili 200€. Ugradi ga u budžet kao legitimnu kategoriju. Branič bez dozvole za trošenje postaje rob anksioznosti.' },
    ...
    { day: 30, task: 'Napravi investicijski plan za sljedeća 3 mjeseca s konkretnim brojevima', tip_: '🎯 Kraj je novi početak', detail: 'Na papir: cilj (iznos, datum), månedni doprinos (€), što novo naučiti, jedna nova akcija. Plan koji zapišeš ima 40% veće šanse realizacije od plana koji postoji samo u glavi.' },
  ],
  vrtlog: [
    ...
    { day: 2,  task: 'Financijski audit: zbrojvi sve prihode i rashode — samo brojke, bez osude', tip_: '📊 5 minuta, bez izgovora', detail: 'Uzmi bankovni izvod ili otvori aplikaciju banke. Zbrojvi prihode, zbrojvi troškove. Nije bitno da je savršeno — bitno je imati konkretan broj. Procjena je uvijek premala. Podaci, ne osjećaji.' },
    ...
    { day: 13, task: 'Provjeri hitni fond: imaš li ga? Koliko je velik vs. cilj?', tip_: '🛡️ Hitni fond je temelj, ne opcija', detail: 'Hitni fond = 3-6 månednih troškova na odvojenom računu bez debitne kartice. Koliko imaš sad? Razlika između cilja i stvarnosti = iznos koji gradiš ovim programom. Bez hitnog fonda, svako iznenađenje ruši plan.' },
    ...
    { day: 26, task: 'Postavi financijsku rutinu za sljedeći måsec — isti dani, iste aktivnosti', tip_: '📅 Rutina = sloboda od odlučivanja', detail: 'Zapiši konkretno: svaki 1. u mj = pregled računa, svaku nedjelju = 5 min pregled troškova. Iste aktivnosti, isti dani, bez razmišljanja. Rutina eliminira trošenje mentalne energije na financijske odluke.' },
    ...
    { day: 29, task: 'Napiši pismo sebi za 6 měseci: gdje želiš biti financijski', tip_: '💌 Vizija budućnosti koja motivira', detail: 'Format: "Dragi ja za 6 mj... Želim da si dostigao/la [X€ ušteđevine], [Y duga manje], [Z navika]." Vizualizacija željene budućnosti aktivira iste moždane krugove kao stvarno iskustvo — moćan motivacijski alat.' },
    { day: 30, task: 'Postavi 3 konkretna cilja za sljedeći măsec s rokovima i akcijama', tip_: '🎯 Kraj je novi početak', detail: 'ŠTO + KOLIKO + DO KADA za svaki od 3 cilja. Unesi ih odmah u kalendar s podsjetnikom. Cilj koji ima datum ima 40% veće šanse realizacije od cilja koji postoji samo kao namjera.' },
  ],
  teoreticar: [
    ...
    { day: 2,  task: 'Financijski audit: zbrojvi SVE prihode i rashode — ODMAH, bez savršenosti', tip_: '📊 Gotovo > savršeno', detail: 'Uzmi bankovni izvod ili otvori aplikaciju. Zbrojvi prihode u jednu kolonu, troškove u drugu. Rezultat = neto prihod. Nije bitno da je savršeno — bitno je imati broj danas. Gotovo uvijek pobjeđuje savršeno.' },
    ...
    { day: 8,  task: 'Definiraj hitni fond: koliko trebaš i gdje ćeš ga čuvati', tip_: '🛡️ Plan u 10 minuta, bez analize', detail: 'Izračunaj: månedni troškovi × 3 = minimalni hitni fond. Za 1.200€/mj = 3.600€ cilj. Odaberi štedni račun bez debitne kartice. Plan za hitni fond u 10 minuta — bez traženja "savršenog računa."' },
    ...
    { day: 19, task: 'Ulož još 50 € u investicije — bez čekanja na "pravi trenutak"', tip_: '🎯 Pravi trenutak je uvijek sada', detail: 'Dodaj još 50€ u investicije danas. Ne čekaj bolje vijesti s tržišta — one nikad ne dolaze u "savršenom" trenutku. Kontinuitet investiranja statistically pobjeđuje timing svaki put na dugim rokovima.' },
    ...
    { day: 22, task: 'Automatiziraj sve što možeš: štednja, investicije, računi — na trajne naloge', tip_: '🤖 Automatizirano > svjesno kontrolirano', detail: 'Napravi popis: štednja, investicije, rate kredita, fiksni računi. Svaki na automatski trajni nalog ako već nije. Manje financijskih odluka svaki måsec = manje mentalne energije potrošene = više za ostalo.' },
    { day: 23, task: 'Pregledaj sve automatizacije — rade li? Prilagodi bez duboke analize', tip_: '🔧 Prilagodba ≠ perfekcionizam', detail: 'Provjeri svaki trajni nalog: je li prošao ovog måseca? Je li iznos ispravan? Prilagodi u jednom pogledu, bez produbljivanja analize. Kratki pregled, jedna prilagodba ako treba — gotovo.' },
    ...
    { day: 30, task: 'Postavi 3 konkretne akcije za sljedeći měsec — bez analize, odmah napiši', tip_: '🎯 Kraj je novi početak', detail: 'Bez analize — odmah zapiši 3 konkretne akcije: broj 1, broj 2, broj 3. Akcija koja je zapisana ima 40% veću šansu realizacije. Akcija koja nije zapisana = nula šanse. Zapiši sada.' },
  ],
}
